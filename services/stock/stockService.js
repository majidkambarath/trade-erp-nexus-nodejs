...
class StockService {
  ...
  static async createStock(data, createdBy) {
    ...
    try {
      ...
      const newItemId = itemId || `ITM${new Date().toISOString().slice(0, 4).replace(/-/g, "")}-${Math.floor(Math.random() * 1000) + 100}`;
...
static async updateStock(id, data, createdBy) {
  ...
  try {
    ...
    const newQuantity = data.currentStock !== undefined ? Number(data.currentStock) : oldQuantity;
    ...
    if (data.reorderLevel) updateData.reorderLevel = Number(data.reorderLevel);
    ...
    if (data.currentStock) updateData.currentStock = Number(data.currentStock);
      ...
      await this.createInventoryMovement(
        {
          ...
          notes: `Manual stock adjustment: ${quantityChange > 0 ? "Added" : "Removed"} ${Math.abs(quantityChange)} units`,
          createdBy,
  ...
  static async processTransactionStock(transactionId, transactionData, createdBy) {
    const session = await mongoose.startSession();
    session.startTransaction();
  ...
  static async reverseTransactionStock(transactionId, transactionData, createdBy) {
    const session = await mongoose.startSession();
    session.startTransaction();
  ...
  static async deleteStock(id) {
    ...
    try {
      ...
      if (movements.length > 0) {
        throw new AppError("Cannot delete stock item with existing inventory movements", 400);
      }
