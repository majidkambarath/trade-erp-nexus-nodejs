...
const { Voucher, LedgerAccount, ExpenseCategory, LedgerEntry } = require("../../models/modules/financial/financialModels");
...
class FinancialService {
  ...
  static async processReceiptVoucher(data, session) {
    const { customerId, linkedInvoices, paymentMode, totalAmount, narration, chequeNo, chequeDate, bankName } = data;
    ...
    const customerAccount = await this.getOrCreateCustomerAccount(customerId, customer.customerName, session);
    entries.push({
  ...
  static async processPaymentVoucher(data, session) {
    const { vendorId, linkedInvoices, paymentMode, totalAmount, notes, chequeNo, chequeDate, bankName } = data;
    ...
    const vendorAccount = await this.getOrCreateVendorAccount(vendorId, vendor.vendorName, session);
    entries.push({
  ...
  static async processExpenseVoucher(data, session) {
    const { expenseCategoryId, totalAmount, description, submittedBy, date } = data;
